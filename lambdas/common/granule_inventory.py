...
INVENTORY_REGEX = re.compile(".*cumulus_rds_granule.*")
...
class InventoriesProgress:
    ...
    @classmethod
    def from_ndjson(cls, ndjson: str, etag: str) -> InventoriesProgress:
...
class InventoryProgressTracker:
    ...
    inventories_progress_name: str = "progress.ndjson"
    ...
    @property
    def _inventory_progress_key(self) -> str:
        return f"{self.inventories_prefix.rstrip('/')}/{self.inventories_progress_name}"

    def create_progress(self) -> InventoriesProgress:
        """Create inventory progress tracking info"""
        inventories = self._list_inventories()
        inventories_progress = InventoriesProgress(
            inventories=[
                InventoryProgress(
        ...
        resp = self.client.put_object(
            Bucket=self.bucket,
            Key=self._inventory_progress_key,
            IfNoneMatch="*",
    ...
    def get_progress(self) -> InventoriesProgress:
        ...
        try:
            ...
            return InventoriesProgress.from_ndjson(
                resp["Body"].read().decode(), resp["ETag"]
            )

    def update_progress(
        self, inventories_progress: InventoriesProgress
    ) -> InventoriesProgress:
